...
/**
 * Decorates date inside date range if it falls on Sunday.
 */
public class CalendarRangeInsideSundayDecorator implements DayViewDecorator {

    private final HashSet<CalendarDay> list = new HashSet<>();
    private final Drawable drawable;
    private final int color;

    public CalendarRangeInsideSundayDecorator(final Context context) {
        drawable = context.getResources().getDrawable(R.drawable.calendar_range_inside_sunday_selector);
        color = ContextCompat.getColor(context, R.color.navy);
    }

    @Override
    public boolean shouldDecorate(CalendarDay day) {
        return list.contains(day) && day.getDate().getDayOfWeek() == DayOfWeek.SUNDAY;
    }
